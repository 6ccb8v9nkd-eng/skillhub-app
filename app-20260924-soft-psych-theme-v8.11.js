...
function rgDemoResultsHtml(){const soft=rgDemoOwnAttempts('soft'),hard=rgDemoOwnAttempts('hard'),typing=rgDemoTypingStats(),softLast=rgDemoLast('soft'),hardLast=rgDemoLast('hard');const date=a=>a?new Date(a.created_at).toLocaleDateString('ru-RU'):'ещё нет';return `<section class="rg-demo-results"><div class="rg-demo-results-head"><div><h3>📊 Мои результаты</h3><p>Ваши личные попытки как РГ. В статистику сотрудников они не попадают.</p></div></div><div class="rg-demo-result-grid"><button class="rg-demo-result-card" onclick="openSoftHub()"><span class="rg-demo-result-icon">💬</span><span><small>Soft Skills</small><strong>${soft.length?(rgDemoAvg('soft')+'%'):'—'}</strong><em>${soft.length} попыток · ${date(softLast)}</em></span><b>Пройти →</b></button><button class="rg-demo-result-card" onclick="openSection('hard')"><span class="rg-demo-result-icon">🧠</span><span><small>Hard Skills</small><strong>${hard.length?(rgDemoAvg('hard')+'%'):'—'}</strong><em>${hard.length} попыток · ${date(hardLast)}</em></span><b>Пройти →</b></button><button class="rg-demo-result-card" onclick="startTyping()"><span class="rg-demo-result-icon">⌨️</span><span><small>Печать</small><strong>${typing.bestCpm!==null?(typing.bestCpm+' зн/мин'):'—'}</strong><em>${typing.count} попыток${typing.lastAcc!==null?' · точность '+typing.lastAcc+'%':''}</em></span><b>Пройти →</b></button></div></section>`}
