console.info('SkillHub build 20260924_v8.14_rg_rs_simplified');
const S={
...
function rgDemoResultsHtml(){
  ...
  const name=S.profile?.name||S.profile?.login||'Руководитель группы';
  const login=S.profile?.login||'';
  ...
  return `<section class="rg-demo-results">
    <div class="rg-demo-results-head"><div><h3>📊 Как РГ увидит результаты сотрудника</h3><p>Здесь вы сами выступаете как сотрудник. После прохождения Soft / Hard / Печати откройте карточку и посмотрите попытки, ответы и ошибки.</p></div></div>
    <div class="rg-demo-self-card">
      <div class="rg-demo-self-main">
        ${sh74AvatarHtml(login,name)}
        <div class="rg-demo-self-person"><b>${esc(name)}</b><small>${esc(login)} · Демо РГ</small><span>${attempts?`${attempts} попыток · последняя ${lastAt}`:'Попыток пока нет'}</span></div>
...
function openRgDemoSelfAttempts(){
  ...
  showModal(`<div class="modal-head"><div><span class="rg-demo-kicker">ДЕМО РГ · КАК КАРТОЧКА СОТРУДНИКА</span><h2>${esc(name)}</h2><div class="meta">${esc(S.profile.login)} · история попыток</div></div><button class="btn secondary" onclick="closeModal()">✕</button></div>
...
function sh814ConfigureRoleNavigation(){
  ...
  if(S.profile.role==='mentor'){
    ...
    apply(['mentor','employees','content','progress'],{
      mentor:'Мой сектор',employees:'Руководители',content:'Контент',progress:'Отчёты'
    });
  }
}
...
function sh815ConfigureRoleNavigation(){
  ...
  if(S.profile.role==='mentor'){
    ...
    apply(['mentor','employees','content','progress','notifications','profile'],{
      mentor:'Мой сектор',employees:'Руководители',content:'Контент',progress:'Отчёты',notifications:'Уведомления',profile:'Профиль'
    });
  }
}
