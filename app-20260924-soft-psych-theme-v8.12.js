...
titles.rgpractice=['Демо РГ','Пройдите путь сотрудника и руководителя на ручном кейсе'];
...
function rgDemoReset(){if(!confirm('Сбросить тестовые ответы в разделе «Демо РГ»? Рабочие данные сотрудников не изменятся.'))return;localStorage.removeItem(rgDemoKey());renderRgPractice();toast('Демо очищено')}
...
function rgDemoResultsHtml(){
  const soft=rgDemoOwnAttempts('soft'),hard=rgDemoOwnAttempts('hard'),typing=rgDemoTypingStats(),softLast=rgDemoLast('soft'),hardLast=rgDemoLast('hard');
  const date=a=>a?new Date(a.created_at).toLocaleDateString('ru-RU'):'ещё нет';
  const scoreLast=a=>a?Math.round(Number(a.score||0)):null;
  const softAvg=rgDemoAvg('soft'),hardAvg=rgDemoAvg('hard');
  return `<section class="rg-demo-results"><div class="rg-demo-results-head"><div><h3>📊 Мои результаты РГ</h3><p>Здесь только ваши собственные попытки. Они не входят в статистику команды.</p></div></div><div class="rg-demo-result-grid">
    <button class="rg-demo-result-card" onclick="openSoftHub()"><span class="rg-demo-result-icon">💬</span><span><small>Soft Skills</small><strong>${softLast?(scoreLast(softLast)+'%'):'—'}</strong><em>${soft.length?`последний · среднее ${softAvg}% · ${soft.length} попыток · ${date(softLast)}`:'Попыток пока нет'}</em></span><b>Пройти →</b></button>
    <button class="rg-demo-result-card" onclick="openSection('hard')"><span class="rg-demo-result-icon">🧠</span><span><small>Hard Skills</small><strong>${hardLast?(scoreLast(hardLast)+'%'):'—'}</strong><em>${hard.length?`последний · среднее ${hardAvg}% · ${hard.length} попыток · ${date(hardLast)}`:'Попыток пока нет'}</em></span><b>Пройти →</b></button>
    <button class="rg-demo-result-card" onclick="startTyping()"><span class="rg-demo-result-icon">⌨️</span><span><small>Печать</small><strong>${typing.bestCpm!==null?(typing.bestCpm+' зн/мин'):'—'}</strong><em>${typing.count?`лучший результат · ${typing.count} попыток${typing.lastAcc!==null?' · последняя точность '+typing.lastAcc+'%':''}`:'Попыток пока нет'}</em></span><b>Пройти →</b></button>
  </div></section>`
}

function renderRgPractice(){
  if(S.profile?.role!=='mentor'){go('mentor');return}
  ...
  page.innerHTML=`<div class="rg-demo-intro"><div><span class="rg-demo-kicker">ТОЛЬКО ДЛЯ РУКОВОДИТЕЛЯ ГРУППЫ</span><h2>Попробуйте весь цикл ручного тренажёра</h2><p>Ручной демо-кейс безопасный и хранится только в вашем браузере. Автоматические Soft, Hard и Печать сохраняют только ваши личные результаты РГ и не смешиваются со статистикой сотрудников.</p></div><button class="btn secondary" onclick="rgDemoReset()">Сбросить демо</button></div>
  ${rgDemoResultsHtml()}
