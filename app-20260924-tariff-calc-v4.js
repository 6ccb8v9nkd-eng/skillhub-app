...
function shTariffCalcIsCase(x){
  const p=x&&x.payload;
  return !!(x&&x.type==='hardcase'&&p&&p.mode==='tariff_calc'&&p.step1&&Number.isFinite(Number(p.step1.answer))&&p.step2&&Array.isArray(p.step2.options));
}
function shTariffCalcFactsHtml(p){
  const facts=Array.isArray(p?.facts)?p.facts:[];
  if(!facts.length)return '';
  return `<div class="sh-tcalc-facts">${facts.map(f=>`<div class="sh-tcalc-fact ${f?.tone?`tone-${esc(f.tone)}`:''}"><span>${esc(f?.label||'')}</span><strong>${esc(f?.value||'')}</strong></div>`).join('')}</div>`;
}
function shTariffCalcActionHtml(p){
  const a=p?.action||{};
  if(!a.title&&!a.amount&&!a.recipient&&!a.method)return '';
  return `<div class="sh-tcalc-wants"><span>КЛИЕНТ ХОЧЕТ</span><b>${esc(a.title||'Провести перевод')}</b><div>${a.amount?`<strong>${esc(a.amount)}</strong>`:''}${a.recipient?`<em>${esc(a.recipient)}</em>`:''}</div>${a.method?`<small>${esc(a.method)}</small>`:''}</div>`;
}
...
function shTariffCalcScenarioHtml(p){
  return `<div class="sh-tcalc-scenario"><div class="sh-tcalc-scenario-head"><div class="sh-tcalc-icon">₽</div><div><span>СИТУАЦИЯ</span><b>${esc(p.label||'Тарифы')}</b></div></div><p>${esc(p.scenario||p.situation||'')}</p>${shTariffCalcFactsHtml(p)}${shTariffCalcActionHtml(p)}</div>`;
}
function startTariffCalc(x){
  S.currentRun={type:'tariff-calc',x,step:1,step1Checked:false,step1Value:null,step1Ok:false,step2Checked:false,step2Selected:null,step2Ok:false,recorded:false};
...
function renderTariffCalc(){
  ...
  if(r.step===1){
    ...
    body=`${shTariffCalcStepPill(2)}<div class="sh-tcalc-client"><span>КЛИЕНТ УТОЧНЯЕТ</span><p>${esc(s2.client||'')}</p></div><div class="sh-tcalc-question"><span>РЕШЕНИЕ СОТРУДНИКА</span><strong>${esc(s2.question||'Какой вариант выгоднее?')}</strong></div><div class="sh-tcalc-options">${opts}</div>${result}`;
  }
  ...
  if(r.step===1&&!r.step1Checked){const inp=$('tariffCalcInput');if(inp)setTimeout(()=>inp.focus(),0)}
}
