...
function shHardScenarioDocumentHtml(p){
  ...
  return `<div class="sh-hard-sc-document"><div class="sh-hard-sc-document-icon">§</div><div class="sh-hard-sc-document-copy"><span>ДОКУМЕНТ / ОГРАНИЧЕНИЕ</span><b>${esc(d.type||'Решение госоргана')}</b><div>${[d.number?`№ ${esc(d.number)}`:'',d.date?`от ${esc(d.date)}`:'',d.received?`поступило ${esc(d.received)}`:''].filter(Boolean).join(' · ')}</div></div></div>`;
}
function shHardScenarioPaymentHtml(p){
  const a=p?.action||{};
  if(!a.title&&!a.amount&&!a.queue&&!a.recipient)return '';
  return `<div class="sh-hard-sc-action"><div class="sh-hard-sc-action-head"><span>ДЕЙСТВИЕ КЛИЕНТА</span><b>${esc(a.title||'Платёж')}</b></div><div class="sh-hard-sc-action-grid">${a.amount?`<div><span>Сумма</span><strong>${esc(a.amount)}</strong></div>`:''}${a.recipient?`<div><span>Кому / куда</span><strong>${esc(a.recipient)}</strong></div>`:''}${a.queue?`<div><span>Очередность</span><strong>${esc(a.queue)}</strong></div>`:''}${a.purpose?`<div><span>Назначение</span><strong>${esc(a.purpose)}</strong></div>`:''}</div></div>`;
}
