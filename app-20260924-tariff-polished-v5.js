...
function renderTariffCalc(){
  ...
  if(r.step===1){
    ...
    body=`${shTariffCalcStepPill(2)}<div class="sh-tcalc-client"><div class="sh-tcalc-client-mark">2</div><div><span>НОВОЕ УСЛОВИЕ ОТ КЛИЕНТА</span><p>${esc(s2.client||'')}</p></div></div><div class="sh-tcalc-question"><span>ВОПРОС СОТРУДНИКУ</span><strong>${esc(s2.question||'Какой вариант выгоднее?')}</strong></div><div class="sh-tcalc-options">${opts}</div>${result}`;
  }
